...
# Overwrite IP for local kubeconfig
IP=`kubectl get nodes -o json | jq -r '.items[] | select(.metadata.name | test("control")|not) | .status.addresses[] | select(.type == "ExternalIP") | .address'`
